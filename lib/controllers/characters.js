...
module.exports = Router()
  ...
  .post('/', async (req, res, next) => {
    try {
      ...
      if (
        ...
        //     spell.level === 0
        //       ? await KnownSpell.insertCantrip(req.user.id, {
        //           charId: character.id,
        //           spellId: spell.id,
        //         })
  ...
  .delete('/:charId', async (req, res, next) => {
    try {
      ...
      await Promise.all(
        knownSpells.map(async (spell) => {
          if (spell.charId === character.id)
            await KnownSpell.deleteKnownSpell(character.id, spell, id);
        })
      );
      res.json(character);
    } catch (error) {
