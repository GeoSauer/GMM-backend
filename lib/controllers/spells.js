...
module.exports = Router()
  ...
  //* returns all spells at or below the character's caster level, regardless of class
  .get('/:charId/all', async (req, res, next) => {
    try {
      const character = await Character.getCharacterById(
        req.user.id,
        req.params.charId
      ...
      const spells = await Spell.getAllSpells(character.casterLvl);
      if (!spells) {
        next();
      }
      res.json(spells);
