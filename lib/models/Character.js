...
module.exports = class Character {
  ...
  static async insertCharacter(
    ...
    const { rows } = await pool.query(
      `
      INSERT INTO characters (user_id, char_name, char_class, char_lvl, char_mod, caster_lvl, prof_bonus, save_dc, attack_bonus, level_1_spell_slots, level_2_spell_slots, level_3_spell_slots, level_4_spell_slots, level_5_spell_slots, level_6_spell_slots, level_7_spell_slots, level_8_spell_slots, level_9_spell_slots)
  ...
  //TODO
  //!removing spell amount tracking for v1
  // static async updateCantripsOrSpellsKnown(userId, characterUpdate) {
  //   const { rows } = await pool.query(
  //     `UPDATE characters
  //     SET cantrips_known = $3, spells_known = $4
  //     WHERE user_id = $1 and id = $2
  //     RETURNING *
  //     `,
  //     [
  //       userId,
  //       characterUpdate.id,
  //       characterUpdate.cantripsKnown,
  //       characterUpdate.spellsKnown,
  //     ]
  //   );
  //   return new Character(rows[0]);
  // }
  ...
  static async updateSpellSlots(userId, characterUpdate) {
    const { rows } = await pool.query(
      `UPDATE characters
