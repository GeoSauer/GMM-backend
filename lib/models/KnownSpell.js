...
module.exports = class KnownSpell {
  constructor({ id, user_id, char_id, spell_id, prepared, known }) {
    this.id = id;
    this.userId = user_id;
    this.charId = char_id;
  ...
  static async insertSpell({ userId, charId, spellId }) {
    const { rows } = await pool.query(
      `INSERT INTO known_spells (user_id, char_id, spell_id, prepared, known)
  ...
  static async insertCantrip({ userId, charId, spellId }) {
    const { rows } = await pool.query(
      `INSERT INTO known_spells (user_id, char_id, spell_id, prepared, known)
  ...
  static async getKnownSpells(userId, charId) {
    const { rows } = await pool.query(
      `SELECT spells.id, spells.index, spells.name, spells.level, spells.school, known_spells.prepared, known_spells.known
      FROM spells
  ...
  static async getPreparedSpells(userId, charId) {
    const { rows } = await pool.query(
      `SELECT spells.id, spells.index, spells.name, spells.level, spells.school, known_spells.prepared
      FROM spells
      LEFT JOIN known_spells
      ON spells.id = known_spells.spell_id
