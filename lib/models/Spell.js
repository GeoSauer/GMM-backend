...
module.exports = class Spell {
  ...
  static async getAvailableSpells(charClass, casterLvl) {
    const { rows } = await pool.query(
      `SELECT id, index, name, level, school, classes, known, from_all
			FROM (
				SELECT DISTINCT spells.id, index, name, level, school, classes, spells.known, known_spells.from_all
				FROM spells
				LEFT JOIN known_spells ON spells.id = known_spells.spell_id
				WHERE $1 = ANY (spells.classes) AND spells.level <= $2
				UNION ALL
				SELECT DISTINCT known_spells.id, index, name, level, school, classes, known_spells.known, known_spells.from_all
				FROM known_spells
				LEFT JOIN spells ON spells.id = known_spells.spell_id
