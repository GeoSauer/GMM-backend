...
module.exports = class User {
  ...
  constructor({ id, username, email, demo, password_hash }) {
    this.id = id;
  ...
  static async insert({ username, email, passwordHash, demo }) {
    const { rows } = await pool.query(
      `
      INSERT INTO users (username, email, password_hash, demo)
      VALUES ($1, $2, $3, $4)
      RETURNING *
    `,
      [username, email, passwordHash, demo]
    );
