...
module.exports = class User {
  ...
  static async insert({
    ...
    const { rows } = await pool.query(
      `
      INSERT INTO users (username, char_name, char_class, char_lvl, caster_lvl, email, password_hash)
      VALUES ($1, $2, $3, $4, $5, $6, $7)
      RETURNING *
    `,
      [username, charName, charClass, charLvl, casterLvl, email, passwordHash]
    );
  ...
  static async updateUserInfo(id, newAttributes) {
    ...
    const { rows } = await pool.query(
      `UPDATE users
      SET username=$2, char_name=$3, char_class=$4, char_lvl=$5, caster_lvl=$6
      WHERE id = $1
      RETURNING *`,
      [
        id,
