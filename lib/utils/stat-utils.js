...
// const calculateCasterLevel = (charLvl, charClass) => {
...
// const calculateSpellsAvailable = (charLvl, charClass, charMod) => {
//   if (!charLvl || !charClass) {
//     throw new Error('Character level or class is undefined');
//   }
...
//     //fallthrough
//     case 'Druid':
//       spellsAvailable = level + mod;
//       break;
//     case 'Paladin':
//       spellsAvailable = mod + Math.floor(level / 2);
//       break;
//     case 'Bard':
//       if (level < 11) {
...
const calculateSaveDCAndAttackBonus = (charLvl, charMod) => {
  const profBonus = calculateProficiencyBonus(charLvl);
  const attackBonus = +profBonus + +charMod;
  const saveDC = +attackBonus + 8;
  return { attackBonus, saveDC };
};

const calculateSpellSlots = (character, slotLevel) => {
  if (!slotLevel || !character) {
    throw new Error('Slot level is undefined');
  }
  character[`level${slotLevel}SpellSlots`] =
    character[`level${slotLevel}SpellSlots`] - 1;
  return character;
};
